  ...
  public void testSimpleCountWithTranspose() {
    ...
    String expectedLogical =
        "LogicalProject(column=[$0], row 1=[$1], row 2=[$2], row 3=[$3], row 4=[$4], row 5=[$5])\n"
            ...
            + "    LogicalProject(_unpivot_value_transpose_=[CAST($3):VARCHAR NOT NULL],"
            + " $f4=[TRIM(FLAG(BOTH), ' ', $2)], $f5=[=($1, 1)], $f6=[=($1, 2)], $f7=[=($1, 3)],"
            + " $f8=[=($1, 4)], $f9=[=($1, 5)])\n"
            + "      LogicalFilter(condition=[IS NOT NULL($3)])\n"
            + "        LogicalProject(c=[$0], _row_number_transpose_=[$1], column=[$2],"
            + " _unpivot_value_transpose_=[CASE(=($2, 'c'), CAST($0):VARCHAR NOT NULL,"
            + " null:NULL)])\n"
    ...
    String expectedSparkSql =
        "SELECT TRIM(`column`) `column`, MAX(CAST(`_unpivot_value_transpose_` AS STRING)) FILTER"
            + " (WHERE `_row_number_transpose_` = 1) `row 1`,"
            + " MAX(CAST(`_unpivot_value_transpose_` AS STRING)) FILTER (WHERE"
            + " `_row_number_transpose_` = 2) `row 2`,"
            + " MAX(CAST(`_unpivot_value_transpose_` AS STRING)) FILTER (WHERE"
            + " `_row_number_transpose_` = 3) `row 3`,"
            + " MAX(CAST(`_unpivot_value_transpose_` AS STRING)) FILTER (WHERE"
            + " `_row_number_transpose_` = 4) `row 4`,"
            + " MAX(CAST(`_unpivot_value_transpose_` AS STRING)) FILTER (WHERE"
            + " `_row_number_transpose_` = 5) `row 5`\n"
            + "FROM (SELECT `t0`.`c`, `t0`.`_row_number_transpose_`, `t1`.`column`, CASE WHEN"
            + " `t1`.`column` = 'c' THEN CAST(`t0`.`c` AS STRING) ELSE NULL END"
            + " `_unpivot_value_transpose_`\n"
            + "FROM (SELECT COUNT(*) `c`, ROW_NUMBER() OVER () `_row_number_transpose_`\n"
            + "FROM `scott`.`EMP`) `t0`\n"
            + "CROSS JOIN (VALUES ('c')) `t1` (`column`)) `t2`\n"
            + "WHERE `t2`.`_unpivot_value_transpose_` IS NOT NULL\n"
  ...
  public void testMultipleAggregatesWithAliasesTranspose() {
    ...
    String expectedLogical =
        "LogicalProject(column=[$0], row 1=[$1], row 2=[$2], row 3=[$3], row 4=[$4], row 5=[$5])\n"
            ...
            + " FILTER $3], row 3_null=[MAX($0) FILTER $4], row 4_null=[MAX($0) FILTER $5], row"
            + " 5_null=[MAX($0) FILTER $6])\n"
            + "    LogicalProject(_unpivot_value_transpose_=[CAST($6):VARCHAR NOT NULL],"
            + " $f7=[TRIM(FLAG(BOTH), ' ', $5)], $f8=[=($4, 1)], $f9=[=($4, 2)], $f10=[=($4, 3)],"
            + " $f11=[=($4, 4)], $f12=[=($4, 5)])\n"
            + "      LogicalFilter(condition=[IS NOT NULL($6)])\n"
            + "        LogicalProject(avg_sal=[$0], max_sal=[$1], min_sal=[$2], cnt=[$3],"
            + " _row_number_transpose_=[$4], column=[$5],"
            + " _unpivot_value_transpose_=[CASE(=($5, 'avg_sal'), NUMBER_TO_STRING($0),"
            + " =($5, 'max_sal'), NUMBER_TO_STRING($1), =($5, 'min_sal'),"
    ...
    String expectedSparkSql =
        "SELECT TRIM(`column`) `column`, MAX(CAST(`_unpivot_value_transpose_` AS STRING)) FILTER"
            ...
            + " STRING) ELSE NULL END `_unpivot_value_transpose_`\n"
            ...
            + "('min_sal'),\n"
            + "('cnt')) `t2` (`column`)) `t3`\n"
            + "WHERE `t3`.`_unpivot_value_transpose_` IS NOT NULL\n"
            + "GROUP BY TRIM(`column`)";
  ...
  public void testTransposeWithLimit() {
    ...
    String expectedLogical =
        "LogicalProject(column=[$0], row 1=[$1], row 2=[$2], row 3=[$3])\n"
            ...
            + "    LogicalProject(_unpivot_value_transpose_=[CAST($6):VARCHAR NOT NULL],"
            + " $f7=[TRIM(FLAG(BOTH), ' ', $5)], $f8=[=($4, 1)], $f9=[=($4, 2)],"
            + " $f10=[=($4, 3)])\n"
            + "      LogicalFilter(condition=[IS NOT NULL($6)])\n"
            + "        LogicalProject(ENAME=[$0], COMM=[$1], JOB=[$2], SAL=[$3],"
            + " _row_number_transpose_=[$4], column=[$5],"
            + " _unpivot_value_transpose_=[CASE(=($5, 'ENAME'), CAST($0):VARCHAR NOT NULL,"
            + " =($5, 'COMM'), NUMBER_TO_STRING($1), =($5, 'JOB'), CAST($2):VARCHAR NOT NULL,"
            + " =($5, 'SAL'), NUMBER_TO_STRING($3), null:NULL)])\n"
            + "          LogicalJoin(condition=[true], joinType=[inner])\n"
    ...
    String expectedSparkSql =
        "SELECT TRIM(`column`) `column`, MAX(CAST(`_unpivot_value_transpose_` AS STRING)) FILTER"
            ...
            + " CAST(`t`.`ENAME` AS STRING) WHEN `t0`.`column` = 'COMM' THEN"
            + " NUMBER_TO_STRING(`t`.`COMM`) WHEN `t0`.`column` = 'JOB' THEN CAST(`t`.`JOB` AS"
            ...
            + "('JOB'),\n"
            + "('SAL')) `t0` (`column`)) `t1`\n"
            + "WHERE `t1`.`_unpivot_value_transpose_` IS NOT NULL\n"
            + "GROUP BY TRIM(`column`)";
  ...
  public void testTransposeWithLimitColumnName() {
    ...
    String expectedLogical =
        "LogicalProject(column_names=[$0], row 1=[$1], row 2=[$2], row 3=[$3])\n"
            ...
            + "      LogicalFilter(condition=[IS NOT NULL($6)])\n"
            + "        LogicalProject(ENAME=[$0], COMM=[$1], JOB=[$2], SAL=[$3],"
            + " _row_number_transpose_=[$4], column_names=[$5],"
            + " _unpivot_value_transpose_=[CASE(=($5, 'ENAME'), CAST($0):VARCHAR NOT NULL,"
            + " =($5, 'COMM'), NUMBER_TO_STRING($1), =($5, 'JOB'), CAST($2):VARCHAR NOT NULL,"
            + " =($5, 'SAL'), NUMBER_TO_STRING($3), null:NULL)])\n"
            + "          LogicalJoin(condition=[true], joinType=[inner])\n"
    ...
    String expectedSparkSql =
        "SELECT TRIM(`column_names`) `column_names`,"
            + " MAX(CAST(`_unpivot_value_transpose_` AS STRING)) FILTER (WHERE"
            + " `_row_number_transpose_` = 1) `row 1`,"
            + " MAX(CAST(`_unpivot_value_transpose_` AS STRING)) FILTER (WHERE"
            + " `_row_number_transpose_` = 2) `row 2`,"
            + " MAX(CAST(`_unpivot_value_transpose_` AS STRING)) FILTER (WHERE"
            + " `_row_number_transpose_` = 3) `row 3`\n"
            ...
            + " NULL END `_unpivot_value_transpose_`\n"
            + "FROM (SELECT `ENAME`, `COMM`, `JOB`, `SAL`, ROW_NUMBER() OVER ()"
            + " `_row_number_transpose_`\n"
            + "FROM `scott`.`EMP`) `t`\n"
            + "CROSS JOIN (VALUES ('ENAME'),\n"
